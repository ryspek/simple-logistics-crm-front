...
const onUserSaved = async (user: User) => {
  if (userToEdit.value) {
    ...
    notify({
      message: `${user.fullname} обновлен`,
      color: 'success',
    })
  } else {
    await usersApi.add(user)
    notify({
      message: `${user.fullname} создан`,
      color: 'success',
    })
  }
}
...
<template>
  ...
  >
    <h1 class="va-h5">{{ userToEdit ? 'Редактирование пользователя' : 'Создание пользователя' }}</h1>
    <EditUserForm
      ref="editFormRef"
      :user="userToEdit"
